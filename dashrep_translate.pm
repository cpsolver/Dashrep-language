...
#  append-multiple-using-prefix-word-list-suffix-to-phrase
        ...
        if ( $action_name eq "append-multiple-using-prefix-word-list-suffix-to-phrase" )
        {
            if ( $number_of_operands != 4 )
            {
                $text_for_value = " " ;
            ...
            $target_phrase_name = $operand_four ;
            $text_for_value = "" ;
            $prefix = "" ;
            if ( defined( $global_dashrep_replacement{ $operand_one } ) )
            {
                $prefix = $global_dashrep_replacement{ $operand_one } ;
            }
            $prefix =~ s/[ \n\t]+//g ;
            $suffix = "" ;
            if ( defined( $global_dashrep_replacement{ $operand_three } ) )
            {
                $suffix = $global_dashrep_replacement{ $operand_three } ;
            }
            $suffix =~ s/[ \n\t]+//g ;
            if ( not( defined( $global_dashrep_replacement{ $operand_two } ) ) )
            {
                $text_for_value = " " ;
                $replacement_text = $text_begin . $text_for_value . $text_end ;
                if ( $global_dashrep_replacement{ "dashrep-warning-trace-on-yes-or-no" } eq "yes" )
                {
                    $global_trace_log .= "{{trace; warning, for action " . $action_name . " , the operand " . $operand_two . " is not defined, so no appending was done" . "}}\n" ;
                }
                next ;
            }
            ...
            if ( $list_of_words_as_text !~ /[^ ]/ )
            {
                ...
                if ( $global_dashrep_replacement{ "dashrep-warning-trace-on-yes-or-no" } eq "yes" )
                {
                    $global_trace_log .= "{{trace; warning, for action " . $action_name . " , the operand " . $operand_two . " does not contain any words, so no appending was done" . "}}\n" ;
                }
                next ;
            }
            @list_of_words = split( /[ \n\t]+/ , $list_of_words_as_text ) ;
            for ( $pointer = 0 ; $pointer <= $#list_of_words ; $pointer ++ )
            {
                $source_phrase_name = $prefix . $list_of_words[ $pointer ] . $suffix ;
            ...
            if ( $global_dashrep_replacement{ "dashrep-action-trace-on-yes-or-no" } eq "yes" )
            {
                $global_trace_log .= "{{trace; did action " . $action_name . " for phrases named " . $operand_one . " and " . $operand_two . " and " . $operand_three . " and " . $operand_four . "}}\n" ;
            }
            $replacement_text = $text_begin . $text_for_value . $text_end ;
            next ;
        }
...
#
        if ( ( $action_name eq "copy-from-phrase-to-phrase-only-word-at-position" ) || ( $action_name eq "copy-words-from-phrase-to-phrase-using-alpha-sort-order-specified-in-phrase" ) )
        {
            if ( $number_of_operands != 3 )
            {
                $text_for_value = " " ;
            ...
            if ( $action_name eq "copy-words-from-phrase-to-phrase-using-alpha-sort-order-specified-in-phrase" )
            {
            ...
            if ( ( $action_name eq "copy-words-from-phrase-to-phrase-using-alpha-sort-order-specified-in-phrase" ) && ( $global_dashrep_replacement{ $operand_three } =~ /[^0-9\.\- ]/ ) )
            {
                $text_for_value = " " ;
                $replacement_text = $text_begin . $text_for_value . $text_end ;
                if ( $global_dashrep_replacement{ "dashrep-warning-trace-on-yes-or-no" } eq "yes" )
            ...
            } else
            {
                ...
                } elsif ( $action_name eq "copy-words-from-phrase-to-phrase-using-alpha-sort-order-specified-in-phrase" )
                {
                    @list_of_words = split( / +/ , $source_text ) ;
