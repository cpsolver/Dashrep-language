...
BEGIN {
    ...
    $global_required_number_of_operands_for_action{ "copy-current-replacement-results-into-phrase-then-escape-from-dashrep" } = 1 ;
    ...
    $global_check_operand_one_is_phrase_name_for_action{ "copy-current-replacement-results-into-phrase-then-escape-from-dashrep" } = "yes" ;
...
#-----------------------------------------------
#  Handle the action:
#  copy-current-replacement-results-into-phrase-then-escape-from-dashrep

        if ( $action_name eq "copy-current-replacement-results-into-phrase-then-escape-from-dashrep" )
        {
            $action_result = " " ;
            ...
            if ( $global_dashrep_replacement{ "dashrep-action-trace-on-yes-or-no" } eq "yes" )
            {
                $global_trace_log .= "{{trace; action " . $action_name . " saved remaining text in phrase " . $operand_one . "}}\n";
            }
#  end of action code
            $replacement_text = $text_begin . $action_result . $text_end ;
            next ;
