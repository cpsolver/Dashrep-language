...
BEGIN {
    ...
    $global_required_number_of_operands_for_action{ "translate-xml-tags-to-dashrep-phrases" } = 2 ;
    ...
    $global_check_operand_one_phrase_is_not_empty_for_action{ "translate-xml-tags-to-dashrep-phrases" } = "yes" ;
    ...
    $global_check_operand_three_is_phrase_name_for_action{ "translate-xml-tags-to-dashrep-phrases" } = "yes" ;
...
#-----------------------------------------------
#  Handle the action:
#  translate-xml-tags-to-dashrep-phrases

#  Handle tags of type "<xyz />" as well as "<xyz>".

        if ( $action_name eq "translate-xml-tags-to-dashrep-phrases" )
        {
            $action_result = " " ;
            $text_string = $global_dashrep_replacement{ $operand_one } ;
            $text_string =~ s/[\n\r\f\t]+/ /sg ;
            $text_string =~ s/ *<!\[CDATA\[ *//sgi ;
            $text_string =~ s/ *\]\]> *//sg ;
#            $text_string =~ s/(<([^ >\!\?\/]+)[^>]* [^ >\=]+=[\"\'][^>]+) +\/>/$1><\/$2>/sgi ;
#            $text_string =~ s/ *<([^ >\/\\]+) */ tag-$1-begin /sg ;
#            $text_string =~ s/[^>]//sg ;
            $global_dashrep_replacement{ $operand_two } = $text_string ;
