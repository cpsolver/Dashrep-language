...
BEGIN {
    ...
    $global_required_number_of_operands_for_action{ "positions-multiple-of-string-in-phrase-within-phrase-put-into-phrase" } = 3 ;
    ...
    $global_required_number_of_operands_for_action{ "use-handler-with-each-word-in-phrase-currently-at-position" } = 3 ;
    ...
    $global_check_operand_three_is_positive_integer_for_action{ "use-handler-with-each-word-in-phrase-currently-at-position" } = "yes" ;
    ...
    $global_check_operand_one_is_phrase_name_for_action{ "positions-multiple-of-string-in-phrase-within-phrase-put-into-phrase" } = "yes" ;
    $global_check_operand_two_is_phrase_name_for_action{ "positions-multiple-of-string-in-phrase-within-phrase-put-into-phrase" } = "yes" ;
    $global_check_operand_three_is_phrase_name_for_action{ "positions-multiple-of-string-in-phrase-within-phrase-put-into-phrase" } = "yes" ;
...
sub dashrep_expand_parameters
{
    ...
    my $repeat_count ;
    my $growing_text_string ;
    my $current_position_number ;
...
#  repeatedly-append-from-phrase-to-phrase-using-repeat-count
        ...
        if ( ( $action_name eq "append-from-phrase-to-phrase" ) || ( $action_name eq "append-from-phrase-to-phrase-no-space" ) || ( $action_name eq "prepend-from-phrase-to-phrase" ) || ( $action_name eq "prepend-from-phrase-to-phrase-no-space" ) || ( $action_name eq "append-word-to-phrase" ) || ( $action_name eq "repeatedly-append-from-phrase-to-phrase-using-repeat-count" ) )
        {
            ...
            } else
            {
                ...
                {
                    if ( exists( $global_dashrep_replacement{ $target_phrase_name } ) )
                    {
                        $text_string =~ s/^ +//s ;
                        $text_string =~ s/ +$//s ;
                ...
                } elsif ( $action_name eq "repeatedly-append-from-phrase-to-phrase-using-repeat-count" )
                {
                    ...
                    $growing_text_string = $text_string ;
                    for ( $count = 2 ; $count <= $repeat_count ; $count ++ )
                    {
                        $growing_text_string .= " " . $text_string ;
                    }
                    $global_dashrep_replacement{ $target_phrase_name } = $growing_text_string ;
                }
                if ( $global_dashrep_replacement{ "dashrep-action-trace-on-yes-or-no" } eq "yes" )
                {
...
#-----------------------------------------------
#  Handle the action:
#  positions-multiple-of-string-in-phrase-within-phrase-put-into-phrase

        if ( $action_name eq "positions-multiple-of-string-in-phrase-within-phrase-put-into-phrase" )
        {
            $action_result = " " ;
            $string_to_find = $global_dashrep_replacement{ $operand_one } ;
            $string_to_search = $global_dashrep_replacement{ $operand_two } ;
...
#  Handle the actions:
#  use-handler-with-each-word-in-phrase
#  use-handler-with-each-word-in-phrase-currently-at-position
#
...
#  its operands are not checked for validity.
        ...
        if ( ( $action_name eq "use-template-with-each-word-in-phrase" ) || ( $action_name eq "use-handler-with-each-word-in-phrase" ) || ( $action_name eq "use-handler-with-each-word-in-phrase-currently-at-position" ) )
        {
            $action_result = " " ;
            ...
            if ( ( $action_name eq "use-handler-with-each-word-in-phrase" ) && ( ( $global_dashrep_replacement{ $operand_two } =~ /^ / ) || ( $global_dashrep_replacement{ $operand_two } =~ / $/ ) ) )
            {
            ...
            {
                $global_dashrep_replacement{ $name_of_parameter_for_handler } = $list_of_words[ 0 ] ;
                $current_position_number = 2 ;
            } elsif ( $action_name eq "use-handler-with-each-word-in-phrase-currently-at-position" )
            {
                $global_dashrep_replacement{ $name_of_parameter_for_handler } = $list_of_words[ $operand_three - 1 ] ;
                $current_position_number = $operand_three + 1 ;
            }
            if ( $current_position_number <= $#list_of_words + 1 )
            {
                $text_to_expand .= "[-use-handler-with-each-word-in-phrase-currently-at-position " . $operand_one . " " . $operand_two . " " . $current_position_number . "-] " ;
            }
            if ( $action_name eq "use-template-with-each-word-in-phrase" )
            {
