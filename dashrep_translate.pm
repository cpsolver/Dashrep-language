...
BEGIN {
    ...
    $global_required_number_of_operands_for_action{ "escape-from-dashrep-replacements" } = 0 ;
...
#-----------------------------------------------
#  Handle the action:
#  escape-from-dashrep-replacements

#  Not yet debugged.

        if ( $action_name eq "escape-from-dashrep-replacements" )
        {
            $action_result = " " ;
            ...
            {
                $global_dashrep_replacement{ "unexpanded-dashrep-replacements-to-resume-after-escape" } = $text_begin . $text_inserted . $text_end ;
                if ( $global_dashrep_replacement{ "dashrep-action-trace-on-yes-or-no" } eq "yes" )
                {
