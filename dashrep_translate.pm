...
#  append-multiple-from-phrases-named-in-pattern-to-phrase
        ...
        if ( $action_name eq "append-multiple-from-phrases-named-in-pattern-to-phrase" )
        {
            ...
            if ( ( $operand_two =~ /^[\-_]/ ) || ( $operand_two =~ /[\-_]$/ ) || ( not( defined( $global_dashrep_replacement{ $operand_two } ) ) ) || ( $global_dashrep_replacement{ $operand_two } !~ /^[^ ]+-[^ ]+$/ ) )
            {
                $text_for_value = " " ;
                $replacement_text = $text_begin . $text_for_value . $text_end ;
                if ( $global_dashrep_replacement{ "dashrep-warning-trace-on-yes-or-no" } eq "yes" )
            ...
            $target_phrase_name = $operand_two ;
            $text_for_value = "" ;
            if ( $global_dashrep_replacement{ $operand_one } =~ /$([^ ]*)ambee-([^ ]*)-amenn([^ ]*)$/ )
            {
                $prefix = $1 ;
                $phrase_name_containing_list = $2 ;
                $suffix = $3 ;
            } else
            {
                $text_for_value = " " ;
            ...
            $suffix =~ s/[ \n\t]+//g ;
            $suffix =~ s/-+$// ;
            $phrase_name_containing_list =~ s/[ \n\t]+//g ;
            $phrase_name_containing_list =~ s/^-+// ;
            $phrase_name_containing_list =~ s/-+$// ;
            if ( not( defined( $global_dashrep_replacement{ $phrase_name_containing_list } ) ) )
            {
                $text_for_value = " " ;
                $replacement_text = $text_begin . $text_for_value . $text_end ;
                if ( $global_dashrep_replacement{ "dashrep-warning-trace-on-yes-or-no" } eq "yes" )
                {
                    $global_trace_log .= "{{trace; warning, for action " . $action_name . " , the operand " . $phrase_name_containing_list . " is not defined, so no appending was done" . "}}\n" ;
                }
                next ;
            }
            $list_of_words_as_text = $global_dashrep_replacement{ $phrase_name_containing_list } ;
            ...
            if ( $list_of_words_as_text !~ /[^ ]/ )
            {
                ...
                if ( $global_dashrep_replacement{ "dashrep-warning-trace-on-yes-or-no" } eq "yes" )
                {
                    $global_trace_log .= "{{trace; warning, for action " . $action_name . " , the operand " . $phrase_name_containing_list . " does not contain any words, so no appending was done" . "}}\n" ;
                }
                next ;
            }
